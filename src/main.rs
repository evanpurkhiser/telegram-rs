...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::History {
            ...
            let result = commands::history::run(
                client.client_id(),
                chat_id,
                limit,
                from_message_id,
        ...
        Commands::Send {
            ...
            let pending_ids =
                commands::send::run(client.client_id(), chat_id, message, file, reply).await?;
        ...
        Commands::Search {
            ...
            client.load_chats().await?;
            let result =
                commands::search::run(client.client_id(), chat_id, query, limit, cli.json).await;
            client.close().await?;
            result
        }
