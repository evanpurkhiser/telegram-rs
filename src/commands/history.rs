...
pub async fn run(
    ...
    let enums::Messages::Messages(history_data) = history_result;
    for msg in history_data.messages.into_iter().rev().flatten() {
...
fn extract_message_info(content: &enums::MessageContent) -> (String, String, Option<Value>) {
    match content {
        ...
        enums::MessageContent::MessagePhoto(photo) => {
            ...
            let size = photo.photo.sizes.last().map(|s| s.photo.size).unwrap_or(0);
