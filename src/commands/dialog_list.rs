...
pub async fn run(client_id: i32, json_output: bool) -> Result<()> {
    let chats_result = convert_tdlib_error(
        functions::get_chats(None, 100, client_id).await
    )?;
    
    let mut dialogs = Vec::new();
    
    // Extract chat_ids from the Chats enum
    if let enums::Chats::Chats(chats_data) = chats_result {
        for chat_id in chats_data.chat_ids {
            let chat_result = convert_tdlib_error(
                functions::get_chat(chat_id, client_id).await
            )?;
            
            // Extract chat data from the Chat enum
